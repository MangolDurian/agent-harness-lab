...
本文件在 s11 基础上增加 worktree 隔离机制：
  ...
  - 队友认领绑定 worktree 的任务时，自动在隔离目录工作

核心设计：
  - 双面架构：控制面 (data/tasks.json) + 执行面 (.worktrees/)
  - 工厂函数创建 worktree-aware handler，无需修改 core/loop.py
  - 队友认领绑定 worktree 的任务时自动隔离

核心约束：core/loop.py 一行不改。

运行方式：python agents/s12_worktree_isolation.py
"""
...
# 配置队友的额外工具（协议工具 + 任务工具 + worktree 工具 + system prompt 后缀）
_team_manager = team.get_manager()
_team_manager.configure_teammate(
    extra_tools=[
        protocols.SCHEMA_SHUTDOWN_RESPONSE,
        protocols.SCHEMA_PLAN_SUBMIT,
        task.SCHEMA_LIST,
        task.SCHEMA_CLAIM,
        task.SCHEMA_COMPLETE,
        worktree.SCHEMA_EXEC,
    ],
    handler_factories={
        "shutdown_response": protocols.make_shutdown_response_handler,
        "plan_submit": protocols.make_plan_submit_handler,
        "task_list": lambda name: task.list_tasks,
        "task_claim": task.make_claim_handler,
        "task_complete": task.make_complete_handler,
        "worktree_exec": lambda name: worktree.exec_in_worktree,
    },
    system_suffix=(
        "\n\n## 团队协议\n"
...
# 启用自主扫描 + 空闲超时
_team_manager.configure_autonomous(enabled=True, timeout=60.0)
...
SYSTEM = (
    ...
    "  - 创建后会在 .worktrees/{name} 下创建独立 git 工作目录，分支 wt/{name}\n"
    "  - 绑定任务后，task_list 会显示 [wt:name] 标记\n"
    "- worktree_remove(name, force?, complete_task?)：删除 worktree\n"
    "  - complete_task=true 会同时完成绑定的任务\n"
    "- worktree_keep(name)：保留 worktree（不被自动清理）\n"
    "- worktree_list()：列出所有 worktree 和状态\n"
    "- worktree_exec(name, command)：在 worktree 中执行命令\n"
    "队友认领绑定 worktree 的任务时，会自动在隔离目录中工作，互不干扰。\n\n"
    ...
    "4. 每个 owner（包括无主任务）最多一个 'in_progress'。\n"
...
TOOLS = [
    ...
    skill.SCHEMA,
    compact.SCHEMA,
    worktree.SCHEMA_CREATE,
    worktree.SCHEMA_REMOVE,
    worktree.SCHEMA_KEEP,
    worktree.SCHEMA_LIST,
    worktree.SCHEMA_EXEC,
]
...
def _print_tool_call(name: str, args: dict, output: str) -> None:
    ...
    if name == "bash" and args.get("run_in_background"):
        ...
        _wt_name = args.get("name", "?")
        _wt_task = args.get("task_id", "")
        task_info = f" → 任务 #{_wt_task}" if _wt_task else ""
        print(f"\033[33m[worktree_create] {_wt_name}{task_info}\033[0m")
    elif name == "worktree_remove":
        _wt_name = args.get("name", "?")
        _force = "force" if args.get("force") else ""
        _complete = "+complete" if args.get("complete_task") else ""
        print(f"\033[33m[worktree_remove] {_wt_name} {_force}{_complete}\033[0m")
    elif name == "worktree_keep":
        _wt_name = args.get("name", "?")
        print(f"\033[33m[worktree_keep] {_wt_name}\033[0m")
    elif name == "worktree_list":
        print(f"\033[33m[worktree_list] 列出所有 worktree\033[0m")
    elif name == "worktree_exec":
        _wt_name = args.get("name", "?")
        _cmd = args.get("command", "")
        print(f"\033[33m[worktree_exec] {_wt_name}: $ {_cmd}\033[0m")
    else:
...
def main() -> None:
    """REPL 主循环。"""
    print("\033[36m== s12: Worktree Isolation ==\033[0m  (q / exit 退出)")
