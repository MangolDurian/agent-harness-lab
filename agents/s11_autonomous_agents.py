...
本文件在 s10 基础上增加自主认领机制：
  - task_claim：队友原子认领无主 pending 任务
  - task_complete：队友完成自己认领的任务
  - 空闲时自动扫描任务板，发现无主任务就认领
  - 超时无工作自动退出

核心设计：
  - 基础设施层扫描（find_claimable） + LLM 层认领（task_claim 工具）
  - 每个 owner 一个 in_progress 槽（多队友可并行）
  - idle_start 时间戳嵌入现有 poll 循环（无新线程）

核心约束：core/loop.py 一行不改。

运行方式：python agents/s11_autonomous_agents.py
"""
...
# 配置队友的额外工具（协议工具 + 任务工具 + system prompt 后缀）
...
def main() -> None:
    """REPL 主循环。"""
    print("\033[36m== s11: Autonomous Agents ==\033[0m  (q / exit 退出)")
