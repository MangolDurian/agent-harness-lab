...
本文件在 s04 基础上新增 load_skill 工具，让 agent 能按需加载技能文件。
技能以 markdown 文件存放在 skills/ 目录，调用 load_skill 时内容作为工具输出
进入对话上下文，模型就能参考这些知识来执行任务。

新增机制：
- tools/skill.py：load_skill 工具，读取 skills/*.md 文件
- skills/ 目录：存放技能定义（git、debug、refactor）

运行方式：python agents/s05_skills.py
"""
...
def main() -> None:
    """REPL 主循环。"""
    print("\033[36m== s05: Skills ==\033[0m  (q / exit 退出)")
